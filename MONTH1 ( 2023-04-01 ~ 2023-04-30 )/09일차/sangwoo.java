...
public class sangwoo{
    ...
    public static String[] solution3(String[] players, String[] callings){
        ...
        Map<Integer,String> rankKeyMap= new HashMap<Integer,String>();
        ...
        for(String str : callings){
            int x = nameKeyMap.get(str);
            String preRank = rankKeyMap.get(x-1);

            nameKeyMap.put(str,x-1);
            nameKeyMap.put(preRank,x);

            rankKeyMap.put(x-1,str);
            rankKeyMap.put(x, preRank);
        }

        for(int i = 0; i < players.length; i++){
            result[i] = rankKeyMap.get(i);
        }
